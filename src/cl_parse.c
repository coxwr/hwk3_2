...
args parse_command_line(const int n_args, char ** arg_array)
{
  ...
  while (cnt < n_args) {
    ...
    else if (!strcmp(arg_array[cnt], "--help") ||
             !strcmp(arg_array[cnt], "-h")) {
      printf("Usage: ./run_md [-N <n_particles>] [-ts <n_timesteps>] [-xyz "
             "<xyz_file_output_frequency>] [-o <thermo_output_frequency>]\n");
      printf("Defaults:\nn_particles: 100\nn_timesteps: "
             "10000\nxyz_file_output_fequency: 100\nthermo_output_frequency: "
             "100\n");
      exit(-1);
    } else {
